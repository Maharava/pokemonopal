...
def get_map_dimensions(map_name):
    map_constants_path = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__)))), 'constants', 'map_constants.asm')
    with open(map_constants_path, 'r') as f:
        for line in f:
            ...
            if line.startswith('map_const') and map_name in line:
                ...
                return width, height
    return 10, 9 # Default to Pallet Town dimensions if not found
...
def main():
    map_name = "PALLET_TOWN" # Default map
    if len(sys.argv) > 1:
        map_name = sys.argv[1].upper()

    map_width_blocks, map_height_blocks = get_map_dimensions(map_name)
    ...
    map_path = os.path.join(project_root, 'maps', f'{map_name.replace("_TOWN", "").replace("_CITY", "").replace("_ISLAND", "").replace("_ROUTE", "").title()}.blk')
